...
final class LocationSearchManager {
    ...
    enum PlacesError: Error {
        case failedToFindPlaceError
        case failedToGetCoordinateError
    }
    ...
    public func findPlaces(query: String, completion: @escaping (Result<[Place], Error>) -> Void) {
        ...
        client.findAutocompletePredictions(
            ...
            guard let results = results, error == nil else {
                completion(.failure(PlacesError.failedToFindPlaceError))
                return
            }
    ...
    public func resolveLocation(for place: Place, completion: @escaping (Result<CLLocationCoordinate2D, Error>)-> Void) {
        client.fetchPlace(
            ...
            guard let googlePlace = googlePlace, error == nil else {
                completion(.failure(PlacesError.failedToGetCoordinateError))
                return
            }
